...
export class ReturnBook implements OnInit {
 ...
 constructor(
  ...
  this.transactionService.getBorrowedTransactions().subscribe({
    next: (transactions) => {
      ...
      const enrichedCalls = transactions.map((t) =>
        forkJoin({
          book: this.bookService.getById(t.bookId),
          member: this.memberService.getById(t.memberId)
        })
      );
  ...
  returnBook(transaction: ITransaction): void {
    // step 1: get book details
    this.bookService.getById(transaction.bookId).subscribe({
      next: (book: Book) => {
        const updatedCopies = book.availableCopies + 1;

        // step 2: update book available copies
        this.bookService.update(book.id, { availableCopies: updatedCopies }).subscribe({
          next: () => {
